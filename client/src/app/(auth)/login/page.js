...
const Login = () => {
    ...
    const btnLight = 'text-dark bg-transparent hover:bg-slate-700 hover:text-white '
    ...
    return (
        <div className='flex flex-col w-1/2 justify-center h-full items-center m-auto'>
            ...
            {/* LOGIN FORM */}
            <form onSubmit={handleSubmit(onSubmit)} className='rounded-md w-fit h-fit flex flex-col justify-center gap-3 p-5'>
                ...
                <div className=" flex w-full justify-center mx-auto rounded-md shadow-sm" role="group">
                    ...
                    {/* EMAIL BUTTON */}
                    <button type="button" className={`inline-flex items-center px-4 py-2 text-sm font-medium border border-dark rounded-r-lg ${(!usernameLogin ? btnDark : btnLight)}`} onClick={() => { setUsernameLogin(false); resetField('username') }}>
                ...
                {/* SIGN IN BUTTON */}
                <button type='submit' className=' bg-dark py-2 px-4 w-fit  mx-auto rounded-[4px] text-white hover:bg-slate-700 transition-colors duration-300'>
                    Sign In
                </button>
            </form>
            ...
            <div className='text-dark mt-2'>
                <span className='text-base leading-4 font-semibold  '>Dont have an account?</span>
                <Link href='/login/register' className='block w-fit text-center mx-auto  underline'>sign up</Link>
            </div>
        </div>
    )
}
